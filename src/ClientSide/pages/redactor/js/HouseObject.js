...
class HouseObject {
    ...
    setVerandaBasementParametrs(verticesCoords) {
        ...
        if (verticesCoords.length > 2 || verandaBasementShapeSelect > 3) {
            ...
            if (Number(verandaBasementShapeSelect.value) <= verandaBasementShapes.length - 2) {
    ...
    changeStageVisability(data) {
        ...
        if( this.viewMode === '2D') {
            ...
            for ( let index = 0; index < stages.length; index++ ) {
                ...
                if (this[ name ]) {
                    if ( this[ name ].plan && (typeof this[ name + 'Existence'] === 'undefined' || this[ name + 'Existence'])) {
                        if ( index <= currentStage ){
                            this[ name ].plan.visible = true;
                            if (name === 'veranda') {putDimensions( this[ name ].plan, this.verandaBasement.vertices, name );}
                            else if (name != 'innerWalls' && name != 'outerWalls') {putDimensions( this[ name ].plan, this[ name ].vertices, name );}
                        } else {
                            this[ name ].plan.visible = false;
                            // if (name === 'veranda') {name = 'verandaBasement';}
                            if (name != 'innerWalls' && name != 'outerWalls') {deleteDimensions(this[ name ].plan, name);}
                        }
                        if (!this.verandaExistence && name === 'veranda') {this.veranda.plan.visible = false;}
                    }
                }
    ...
    changePointsVisability( data ) {
        ...
        if (stageName === 'basement' || stageName === 'veranda') {
            ...
            if (this.veranda.points) {
                this.veranda.points.visible = mode === 'veranda' ? true : false;
                this.verandaWalls.plan.visible = mode === 'veranda' ? false : true;
            }
        }
        else if (stageName === 'innerWalls') {
            ...
            case 'verandaExistence' :
                if (this.verandaExistence) {
                    if (this.verandaModel) {
                        ...
                        if (appState.house.viewMode === '2D') {
                            this.veranda.plan.visible = true;
                            putDimensions( this.veranda.plan, this.verandaBasement.vertices, 'veranda' );
                        }
                    } else {
                        this.build( {stageName: 'veranda'});
                        if (appState.house.viewMode === '2D') {putDimensions( this.veranda.plan, this.verandaBasement.vertices, 'veranda' );}
                    }
                    if (this.floors === 1) {
                        this.roofModel.visible = false;
                        this.verandaRoofModel.visible = true;
                    ...
                    if (this.verandaModel) {
                        ...
                        this.veranda.plan.visible = false;
                        deleteDimensions(this.veranda.plan, 'veranda');
    ...
    showCurrentFloor(floorName, stageName) {
        ...
        switch (floorName) {
            ...
            case '1':
                ...
                this.roofModel.visible=false;
                if (this.verandaExistence && stageName === 'interior') {
                    this.verandaModel.visible = true;
    ...
    showModels(floorName, viewMode) {
        ...
        if (viewMode === '2D') {
            ...
            this.floorNames.map((floor, i) => {
                if (i<=floorIndex) {
                    ...
                    if (this.models[floor]) {
                        this.models[floor].visible = true;
                        let index = Number(floor) - 2;
                        if (index>=0) {
                            this.floorsModel.children[index].children[1].children[0].visible = false;
                            this.floorsModel.children[index].children[1].children[2].visible = false;
                        }
                    };
                } else {
                    this.house3d.children.map(child => {
            ...
            this.outerWallsModel.children[0].visible = false;
            this.outerWallsModel.children[2].visible = false;
        }        
    }
    hideModels() {    
        if (!this.models) {return;}    
        this.floorNames.map((floor, i) => {
            ...
            if (this.models[floor]) {
                this.models[floor].visible = false;
                let index = Number(floor) - 2;
                if (index>=0) {
    ...
    buildVerandaWalls2D() {
        this.veranda.plan.remove(this.verandaWalls.plan);
        this.verandaWalls.plan = drawPolygon(this.verandaWalls, [0,0,-this.verandaWalls.height]);
        const material2D = new THREE.MeshPhongMaterial({color: new THREE.Color('rgb(45,45,45)')});
        this.verandaWalls.plan.material = material2D;
        this.verandaWalls.plan.name = 'verandaWalls2d';
        this.veranda.plan.add(this.verandaWalls.plan);
        this.veranda.plan.add(this.verandaWalls.plan);
    }
