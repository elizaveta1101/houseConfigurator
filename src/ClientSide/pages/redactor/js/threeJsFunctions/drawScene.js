...
function drawScene(
  ...
  if (init) {
    // если запрос инициализации, то выставляем предустановленные значения
    ...
    // иначе берем значения из переданных параметров
    if (bckgd === 0) {
    ...
    if (camNear != null) {
      appState.sceneCamera.near = camNear
      appState.sceneCamera.updateProjectionMatrix()
    }
    if (camFar != null) {
      appState.sceneCamera.far = camFar
      appState.sceneCamera.updateProjectionMatrix()
    }
  ...
  function animate() {
    ...
    if (appState.sceneCamera.rotation.x > Math.pi / 2) {
      appState.sceneCamera.rotation.x = Math.pi / 2
    }
    if (appState.sceneCamera.position.z < 0) {
      appState.sceneCamera.position.z = 0
    }
