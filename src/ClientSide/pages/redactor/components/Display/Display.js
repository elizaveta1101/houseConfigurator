...
class Display extends React.Component {
  ...
  fieldOnChange(event) {
    ...
    if (
      ...
      stages[index].fields[num].type === 'checkbox'
    ) {
    ...
    if (name.indexOf('Material') > -1) {
      ...
      if (stageName === 'innerWalls') {
        let floorSelect = stages
          .filter((el) => el.name === 'interior')[0]
          .fields.filter((el) => el.fieldId === 'editableFloor')[0]
        floorSelect.value = value
      } else if (stageName === 'interior') {
        let floorSelect = stages
          .filter((el) => el.name === 'innerWalls')[0]
          .fields.filter((el) => el.fieldId === 'editableFloor')[0]
        floorSelect.value = value
      }
      appState.changeState('floorVisibility', { value })
  ...
  checkDrawBtnVisibility(index) {
    if (stages[index].condition) {
      ...
      if (conditionFieldId) {
        ...
        let value = stages[index].fields.filter((el) => el.fieldId === conditionFieldId)[0].value
  ...
  changeFloorList(name, value) {
    let House = appState.house
    let editableFloorSelect = stages
      .filter((el) => el.name === 'innerWalls')[0]
      .fields.filter((el) => el.fieldId === 'editableFloor')[0]
    if (name === 'floors') {
      //обновление списка
  ...
  editObject() {
    if (stages[this.state.stageId].name === 'basement') {
      ...
      if (!this.state.editBtn.clicked) {
        if (
          linePointsCoords[0] === 0 &&
          linePointsCoords[1] === 0 &&
          linePointsCoords[2] === 0 &&
          linePointsCoords.length < 4
        ) {
          appState.editMode = 'add'
          this.setState({
            editBtn: {
        ...
        if (linePointsCoords.length < 9) {
          ...
          appState.changeState('endAddVertices')
        }
        appState.editMode = 'N'
      ...
      if (!this.state.editBtn.clicked) {
        ...
        appState.changeState('planEditMode', {
          mode: 'veranda',
          stageName: stages[this.state.stageId].name,
        })
        this.set2d()
      } else {
        if (linePointsCoords.length < 9) {
          ...
          appState.changeState('endAddVertices', { stageName: stages[this.state.stageId].name })
        }
        appState.editMode = 'N'
  ...
  chooseCard(event) {
    ...
    let materialField = stages[this.state.stageId].fields.filter(
      (el) => el.fieldId.indexOf('Material') > -1
    )[0]
