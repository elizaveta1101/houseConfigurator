    ...
    getUpVertices() {
        if (this.vertices.length !== 0) {
            ...
            for (let i = 0; i < this.vertices.length; i += 3) {
                ...
                this.upVertices.push(this.vertices[i], this.vertices[i + 1], this.height);
            }
            if (this.innerVertices.length !== 0) {
                for (let i = 0; i < this.innerVertices.length; i += 3) {
                    this.upVertices.push(this.innerVertices[i], this.innerVertices[i + 1], this.height);
                }
            }
        } else {
    ...
    getInnerVertices() {
        if (this.vertices.length !== 0) {
            ...
            for (let i = 3; i < this.vertices.length - 3; i += 3) {
                ...
                this.innerVertices.push(result[0], result[1], this.vertices[2]);
            }
            ...
            result = extraFunctions.getBisectorPoint([x1 - x2, y1 - y2], [x3 - x2, y3 - y2], x2, y2, width, round);
            this.innerVertices.unshift(result[0], result[1], this.vertices[2]);
            this.innerVertices.push(result[0], result[1], this.vertices[2]);
        } else {
